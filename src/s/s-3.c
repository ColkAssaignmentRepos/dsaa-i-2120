...
void add_guard(person_p *list, int length, char *query);

const char NAMES[LIST_LENGTH][MAX_NAME_LENGTH] = {"Chino", "Cocoa", "Maya", "Megu", "Mocha", "GUARD"};
...
int main() {
    ...
    add_guard(list, LIST_LENGTH, query);
...
void add_guard(person_p *list, int length, char *query) {
    strcpy(list[length - 1].name, query);
}
